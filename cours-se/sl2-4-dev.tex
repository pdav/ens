...
\begin {frame} {Évolutions}
	...
	\item Exemple : fonctions supplémentaires sur FreeBSD
	    \ctableau {\fD} {|l|l|} {
		\rc \code {identify} & reconnaître le périphérique \\
		\rc \code {probe} & vérifier si le périphérique est connecté \\
		\rc \code {attach} & insérer le périphérique dans
		    les structures de données du noyau \\
		\rc \code {detach} & retirer le périphérique \\
		\rc \code {shutdown} & prépare le retrait \\
		\rc \code {suspend} & appelée lors de la mise en veille \\
		\rc \code {resume} & appelée lors de la reprise \\
	    }
    \end {itemize}
\end {frame}
