...
\begin {frame} {Structure des disques -- Partitions DOS}
	...
	\item secteur 0 = MBR (\textit {Master Boot Record\/})
	    ...
	    \ctableau {\fE} {|r|l|} {
		\rc 446 octets & code de démarrage \\
		\rc 4 $\times$ 16 octets & table de 4 partitions « primaires » \\
		\rc 2 octets & nombre magique \\
	    }
	...
	\item partition primaire avec type « Extended » \implique elle
		...
		\item chaque partition secondaire débute par un
		    ...
		    \ctableau {\fE} {|r|l|} {
			\rc 446 octets & inutilisé \\
			\rc 16 octets & description de la partition \\
			\rc 16 octets & lien vers la partition secondaire suivante \\
			\rc 32 octets & inutilisé \\
			\rc 2 octets & nombre magique \\
		    }
	    \end {itemize}
...
\begin {frame} {Buffer cache}
    ...
    \ctableau {\fD} {|l|c|c|c|} {
	\rc & \textbf {cache CPU} & \textbf {buffer cache} & \textbf {cache disque} \\
	\rc \textbf {géré par}
	    & CPU & noyau & firmware disque \\
	\rc \textbf {cache entre...}
	    & mémoire centrale & disque & secteurs du disque \\
	\rc \textbf {... et}
	    & SRAM CPU & mémoire centrale & mémoire interne disque \\
	\rc \textbf {« invisible »\footnote {\fD la plupart du temps...
