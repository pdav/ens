...
\begin {frame} {Évolutions}
	...
	\item Exemple : fonctions supplémentaires sur FreeBSD
	    \ctableau {\fD} {|l|l|} {
		\rca \code {identify} & reconnaître le périphérique \\
		\rca \code {probe} & vérifier si le périphérique est connecté \\
		\rca \code {attach} & insérer le périphérique dans
		    les structures de données du noyau \\
		\rca \code {detach} & retirer le périphérique \\
		\rca \code {shutdown} & prépare le retrait \\
		\rca \code {suspend} & appelée lors de la mise en veille \\
		\rca \code {resume} & appelée lors de la reprise \\
	    }
    \end {itemize}
\end {frame}
