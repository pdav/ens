...
\begin {frame} {Continuons l'historique}
	...
	\item Burroughs MCP (1961)
	    \begin {itemize}
		\item conçu pour l'ordinateur Burrouggs B5000
...
\begin {frame} {Unix}
	...
	\item Unix
		...
		\begin {tabular} {|l|l|} \hline
		    ...
		    \rcb 1971 & achat PDP-11/20 en échange d'un formatteur de texte \\
...
\begin {frame} {POSIX -- Paramètres de type « pointeur »}
	...
	\item Exemples :
	    \begin {itemize}
		\item \code {int stat (char *path,
				\alert {struct stat *stbuf})}
		    \\
		    Retourne dans l'emplacement repéré par \code {stbuf}
		    les attributs du fichier
		\item \code {pid\_t wait (\alert {int *status})}
		    \\
		    Place dans l'emplacement repéré par \code {status}
...
\begin {frame} {POSIX -- Paramètres de type « pointeur »}
	...
	\rca \multicolumn {1} {|c|} {Correct}
	    & \multicolumn {1} {c|} {\alert {Faux}}
	    \\ \hline
	\rcb
