...
Les primitives {\tt stat}, {\tt lstat} et {\tt fstat} lisent la partie
système d'un fichier (son {\it inode}) et placent ces informations dans
la structure pointée par {\tt buf}. Le fichier peut être spécifié
par son nom ({\tt stat} et {\tt lstat}) ou par le descripteur de fichier
qui lui est associé ({\tt fstat}).

Lorsque le fichier est un lien symbolique, {\tt stat} et {\tt fstat}
renvoient des informations sur la cible du lien alors que {\tt lstat}
...
\begin {itemize}
	...
	\begin {tabular} {|ll|} \hline
	    ...
	    \tt S\_ISLNK({\em mode}) & lien symbolique \\
	    \hline
	\end {tabular}
...
Les primitives {\tt truncate} et {\tt ftruncate} tronquent un
...
\primitive {clock\_getres, clock\_gettime, clock\_settime} --- Heure pour extensions temps réel
    \label {clockgettime}
    \index{{\tt clock\_getres}}
    \index{{\tt clock\_gettime}}
    \index{{\tt clock\_settime}}
...
fonction {\tt fprintf} écrit sur le flux {\tt flux}, la fonction \texttt
...
fonction {\tt snprintf} place le résultat dans la chaîne de caractères
{\tt str} en vérifiant le non-débordement de cette chaîne.
...
suivant le format {\tt format}.  Le format est une chaîne de caractères
...
{\tt \%} [\textit {flags}] [\textit {largeur-min}] [\verb|.| \textit {precision}] [\textit {taille}] \textit {conversion}
...
\begin {itemize}
	...
	\begin {tabular} {|c|l|} \hline
		...
		\\ \hline
	    \verb|g| & conversion d'un flottant en {\tt \%d},
		{\tt \%e} ou {\tt \%f} suivant le cas
		\\ \hline
		...
		\\ \hline
	    \verb|%| & affichage d'un caractère {\tt \%}
		\\ \hline
	\end {tabular} 
	\end {quote}
...
La fonction {\tt putchar(c)} est équivalente à {\tt putc(c,stdout)}.
...
La fonction {\tt perror} affiche sur la sortie d'erreur standard le
message correspondant, précédé de la chaîne {\tt str} (qui est
typiquement le nom du programme courant).

La fonction {\tt strerror} retourne un pointeur sur une chaîne de
caractères contenant le message correspondant à l'erreur de numéro {\tt
...
La fonction {\tt psignal} affiche sur la sortie d'erreur standard une
description du signal numéro \texttt {signum} précédée de la chaîne
{\tt str}.

La fonction {\tt strsignal} retourne un pointeur sur une chaîne de
...
La fonction {\tt nanosleep} met le processus en attente pendant un
temps spécifié par le paramètre {\tt duree}. Pour la définition de
