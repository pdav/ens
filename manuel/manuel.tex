...
Si le paramètre {\tt adresse} est non nul, 16 bits
...
la fonction de librairie {\tt mkfifo} (voir page~\ref {mkfifo}).
...
Cette primitive renvoie l'heure courante, ou -1
(ou plus exactement \verb|((time_t)-1)|) en cas d'erreur.
...
\separation
\primitive {recv} --- lecture de données
    \index{{\tt recv}}
...
\separation
\primitive {recvfrom} --- lecture de donnéees
    \index{{\tt recvfrom}}
...
spécifiés par {\tt readdescs} ait des données en attente, qu'un
des descripteurs spécifiés apr {\tt writedescs} soit prêt à
recevoir des données, qu'un des descripteurs spécifiés par
{\tt exceptdescs} exhibe une condition exceptionnelle, ou
que la durée spécifiée par {\tt timeout} soit écoulée.
...
Si un masque ou {\tt timeout} n'est pas utile, le pointeur
...
\separation
\primitive {send} --- émission de données
    \index{{\tt send}}
...
\separation
\primitive {sendto} --- émission de données
    \index{{\tt sendto}}
...
La primitive {\tt profil} valide la mesure des
temps d'exécution ({\it process profiling}), qui aide
à identifier les portions de programme prenant le
plus de temps. La valeur du pointeur programme est
lue à chaque top d'horloge (tous les 1/50
secondes) pour incrémenter un compteur dans la
zone {\tt buf}. {\tt Echelle} et {\tt offset} sont utilisés
pour déterminer le mot à incrémenter.
...
int getaddrinfo (
    const char *nom, const char *service,
    const struct addrinfo *indic,
...
int getnameinfo (
    const struct sockaddr *adr, socklen_t longadr,
    char *nom, size_t longnom,
    char *service, size_t longserv,
    int flags)
\end{verbatim}
\end {quote}
...
\begin {quote}
\begin {verbatim}
int pthread_kill (pthread_t thread, int numsig)
\end{verbatim}
\end {quote}

Cette fonction envoie le signal \texttt {numsig} au thread indiqué par le
paramètre \texttt {thread}.  Si \texttt {numsig} vaut 0, aucun signal
...
courante). La structure \texttt {timespec} contient les champs suivants~:
\begin {quote}
    \small
\begin {verbatim}
struct timespec
